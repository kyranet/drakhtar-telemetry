...
function Assert-MsBuildPath([string] $MsBuild) {
	if (($MsBuild -Eq "") -Or !(Test-Path -LiteralPath $MsBuild -PathType Leaf)) {
		...
		Write-Host ".\scripts\windows-build.ps1 `"Path\To\MSBuild.exe`""          -ForegroundColor Cyan   -NoNewline
		Write-Host " or "                                                         -ForegroundColor Yellow -NoNewline
		Write-Host ".\scripts\windows-build.ps1 -MsBuild `"Path\To\MSBuild.exe`"" -ForegroundColor Cyan   -NoNewline
...
$local:BuildSolution = Join-Path -Path $BuildFolder -ChildPath "telemetry.sln"
